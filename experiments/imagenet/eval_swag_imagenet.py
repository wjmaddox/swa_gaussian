...
parser.add_argument('--save_path_swag', type=str, default=None, required=True, help='path to SWAG npz results file')
...
swa_nll = -np.mean(np.log(swa_predictions[np.arange(swa_predictions.shape[0]), targets] + eps))
...
for i in range(args.num_samples):
    ...
    print('SWAG Sample %d/%d. Accuracy: %.2f%% NLL: %.4f' % (i + 1, args.num_samples, accuracy * 100, nll))

    swag_predictions += predictions

    ens_accuracy = np.mean(np.argmax(swag_predictions, axis=1) == targets)
    ens_nll = -np.mean(np.log(swag_predictions[np.arange(swag_predictions.shape[0]), targets] / (i + 1) + eps))
    print('Ensemble %d/%d. Accuracy: %.2f%% NLL: %.4f' % (i + 1, args.num_samples, ens_accuracy * 100, ens_nll))

swag_predictions /= args.num_samples

swag_accuracy = np.mean(np.argmax(swag_predictions, axis=1) == targets)
swag_nll = -np.mean(np.log(swag_predictions[np.arange(swag_predictions.shape[0]), targets] + eps))
