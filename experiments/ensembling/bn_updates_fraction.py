...
parser = argparse.ArgumentParser(description='Dependence of the faction iof samples for BN updates')
...
parser.add_argument('--N', type=int, default=11, metavar='N', help='grid size (default: 25)')
parser.add_argument('--S', type=int, default=30, metavar='S', help='number of samples for SWAG (default: 30)')
...
swag_model = SWAG(model_cfg.base, no_cov_mat=not args.cov_mat, max_num_models=args.swag_rank, *model_cfg.args, num_classes=num_classes, **model_cfg.kwargs)
...
for i, fraction in enumerate(fractions):
    ...
    swag_nlls[i] = -np.mean(np.log(predictions[np.arange(predictions.shape[0]), targets] + eps))

    run_time = time.time() - start_time
    values = [fraction * 100.0, swa_accuracies[i], swa_nlls[i], swag_accuracies[i], swag_nlls[i], run_time]
